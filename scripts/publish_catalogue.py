...
def publish(api, push, receipt):
    ...
    if existing:
        require(existing["head"]["ref"] == branch and existing["head"]["repo"]["full_name"] == REPO
                and existing["base"]["ref"] == "main" and existing["base"]["repo"]["full_name"] == REPO
    ...
    require(live["head"]["sha"] == head and live["head"]["ref"] == branch
            ...
            and live["base"]["ref"] == "main" and live["base"]["repo"]["full_name"] == REPO
            and live["state"] == "open"
...
class Transport:
    ...
    def __call__(self, path, method="GET", data=None):
        ...
        require(method == "GET" or (method in {"POST", "PATCH"} and path.startswith(BASE + "/pulls")),
